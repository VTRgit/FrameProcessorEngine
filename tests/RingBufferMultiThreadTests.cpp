...
TEST_F(RingBufferMultithreadTests, MultiThreadedPush) {
    ...
    for (int i = 0; i < NUM_THREADS; ++i) {
        threads.emplace_back(&RingBufferMultithreadTests::pushElements, this, i, ELEMENTS_PER_THREAD);
    }
...
TEST_F(RingBufferMultithreadTests, MultiThreadedPop) {
    ...
    for (int i = 0; i < NUM_THREADS; ++i) {
        threads.emplace_back(&RingBufferTest<int>::popElements, this, std::ref(popped), ELEMENTS_PER_THREAD);
    }
